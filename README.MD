...
- [x] 宏支持<br>已完成对宏的主要兼容与运算，不过这在多态all-units.template存在一些问题。
...
- [ ] 不规范的持有<br>持有mod-info.txt，背景音乐，地图以及地图图像，使打包出错或造成多次复制，以及被移除的键。
...
- [ ] 多态宏<br>对多态宏达重复追踪尚未实现，无论是否进行复制节，你都需要在节定义有效的键引用，来表达该内容不是重复的。。

## 配置
配置路径移动到/sdcard/Android/data/rust.rwp/files，避免卸载残留。
